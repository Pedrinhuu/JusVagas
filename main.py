...
async def executar_busca():
    ...
    for query in PALAVRAS_CHAVE:
        for cidade in CIDADES:
            resultado = await buscar_infojobs(query, cidade)
            todas_vagas.extend(resultado)

            resultado = await buscar_linkedin(query, cidade, incluir_remoto=BUSCAR_REMOTO)
            todas_vagas.extend(resultado)

        resultado = await buscar_vagas_com(query, incluir_remoto=BUSCAR_REMOTO)
        todas_vagas.extend(resultado)

        resultado = await buscar_trabalha_brasil(query, incluir_remoto=BUSCAR_REMOTO)
        todas_vagas.extend(resultado)
